...
def test_put_v1_account_password(account_helper, prepare_user):
    ...
    account_helper.change_password(login=user.login, email=user.email, old_password=user.password,
                                   new_password=new_password)
    account_helper.auth_client(login=user.login, password=new_password)
